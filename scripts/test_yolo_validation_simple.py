#!/usr/bin/env python3
"""
Test YOLO API on validation dataset using YOLO format labels.
"""
...
def call_yolo_api(api_url: str, image_path: Path) -> Dict:
    """Call YOLO API for detection."""
    try:
        with open(image_path, "rb") as f:
            files = {"file": (image_path.name, f, "image/jpeg")}
            response = requests.post(f"{api_url}/v1/detect", files=files, timeout=30)
            response.raise_for_status()
        ...
        return {
            ...
            "threats": [item['name'] for item in items if item['name'].lower() in [t.lower() for t in THREATS]],
            "success": True
        }
    except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description="Test YOLO API on validation dataset")
    ...
    for i, image_path in enumerate(image_files):
        ...
        # Call API
        api_result = call_yolo_api(args.api_url, image_path)
    ...
    results_file = output_dir / f"yolo_validation_{total}samples.json"
