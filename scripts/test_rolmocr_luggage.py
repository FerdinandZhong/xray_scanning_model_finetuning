#!/usr/bin/env python3
"""
Test RolmOCR model on Luggage X-ray dataset and compute confusion matrix.
...
def query_rolmocr(
    ...
    """
    Query RolmOCR with an X-ray image using structured output.
    """
    # Encode image
    base64_image = encode_image(image_path)
    
    # Create prompt - RolmOCR is an OCR model, so keep it simple
    prompt = f"""Analyze this X-ray image and identify all objects visible.

Classify objects into these categories:
{', '.join(CATEGORIES)}

Provide a list of detected items with their names and confidence scores."""
...
def plot_confusion_matrix(
    ...
    if normalize:
        ...
        title = 'Normalized Confusion Matrix (RolmOCR on Luggage X-ray)'
        fmt = '.2f'
        vmax = 1.0
    else:
        cm_norm = cm
        title = 'Confusion Matrix (RolmOCR on Luggage X-ray)'
        fmt = 'd'
        vmax = None
...
def save_results(
    ...
    # Save JSON
    json_path = output_dir / f"rolmocr_luggage_results.json"
    ...
    # Save text report
    report_path = output_dir / f"rolmocr_luggage_report.txt"
    with open(report_path, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("RolmOCR on Luggage X-ray Dataset - Test Results\n")
        f.write("=" * 80 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Test RolmOCR on Luggage X-ray dataset"
    )
    parser.add_argument(
        '--base-url',
        type=str,
        required=True,
        help='RolmOCR API base URL'
    )
    parser.add_argument(
        '--model-id',
        type=str,
    ...
    parser.add_argument(
        '--output-dir',
        type=Path,
        default=Path('test_results/rolmocr_luggage'),
    ...
    print("=" * 80)
    print("RolmOCR Luggage X-ray Testing")
    print("=" * 80)
    ...
    for image_path, ground_truth, idx in tqdm(samples):
        # Query model
        detection_result = query_rolmocr(client, image_path, args.model_id)
